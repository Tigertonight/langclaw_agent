...
import type { SkillDefinition, SkillRegistryConfig } from "../types/agent-contracts.js";

interface SkillRegistryStoreOptions {
  workspaceDir?: string;
  builtinDir?: string;
}
...
export class SkillRegistryStore {
  ...
  constructor({ workspaceDir = "workspace/skills", builtinDir = "skills" }: SkillRegistryStoreOptions = {}) {
  ...
  async listConfig(): Promise<SkillRegistryConfig> {
    if (!existsSync(this.registryFile)) {
      return { installed: [], overrides: {} };
    }
    try {
      return JSON.parse(await readFile(this.registryFile, "utf8")) as SkillRegistryConfig;
    } catch {
      return { installed: [], overrides: {} };
    }
  }

  async saveConfig(config: SkillRegistryConfig): Promise<void> {
    await mkdir(this.workspaceDir, { recursive: true });
    await writeFile(this.registryFile, JSON.stringify(config, null, 2), "utf8");
  }

  async installFromLocalDir(sourcePath: string): Promise<SkillRegistryConfig["installed"][number]> {
  ...
  async setEnabled(skillId: string, enabled: boolean): Promise<SetEnabledResult> {
    const config = await this.listConfig();
  ...
  async listInstalledEntries(): Promise<SkillRegistryConfig["installed"]> {
    const config = await this.listConfig();
    return config.installed ?? [];
  }

  async applyOverrides(definition: SkillDefinition): Promise<SkillDefinition> {
    const config = await this.listConfig();
