import { resolveEntities, type ResolvedEntities } from "./entity-resolver.js";
...
interface CustomerQueryInput {
  question: string;
  entities: ResolvedEntities;
  operation: string;
}

interface LeaveRecordQueryInput extends CustomerQueryInput {
  user: UserContext;
}
...
export async function parseBusinessQuery({ user, question, history = [], conversationContext }: ParseBusinessQueryInput): Promise<QueryIR | null> {
  ...
  if (isDealerQuestion(question)) {
    return parseDealerQuery({ question, operation, forcedTarget: undefined });
  }
...
export async function planDealerMultiQuery({ question, operation = inferOperation(question) }: { question?: string; operation?: string } = {}): Promise<QueryIR[] | null> {
...
function parseDealerQuery({ question, operation, forcedTarget }: DealerQueryInput): QueryIR {
...
function normalizeDealerOperation({ target, question, operation }: DealerQueryInput & { target: string }): string {
  const text = String(question ?? "");
...
function parseOrganizationQuery({ user, question, entities, operation }: OrganizationQueryInput): QueryIR {
...
function parseCustomerQuery({ question, entities, operation }: CustomerQueryInput): QueryIR {
  const filters: QueryFilter[] = [];
...
function parseLeaveRecordQuery({ user, question, operation, entities }: LeaveRecordQueryInput): QueryIR {
  const filters: QueryFilter[] = [];
...
function isContextContinuationForTarget(context: ParseBusinessQueryInput["conversationContext"], target: string): boolean {
  return context?.continuation?.is_likely_continuation === true
    && context?.last_task?.target === target;
}

function extractRecentStartDate(question: unknown): string | null {
  const text = String(question ?? "");
...
function parseChineseNumber(token: unknown): number | null {
...
function extractSalesReportDepartment({ user, question }: { user: UserContext; question: string }): string | undefined {
...
function extractMonthToken(question: unknown): string | null {
