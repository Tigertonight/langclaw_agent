...
const REQUIRED_SLOTS = ["leave_type", "start_time", "end_time", "reason"];

export class LeaveRequestScenario {
  constructor({ toolRegistry }) {
    this.intent = "leave_request";
    this.toolRegistry = toolRegistry;
  }

  async run({ user, message, session }) {
  ...
  async submit({ user, state }) {
    ...
    const result = await this.toolRegistry.execute(call, { user });
    await appendAuditEvent({
      type: "tool.executed",
      tool: call.name,
      user_id: user.id,
      risk_level: this.toolRegistry.describe(call.name)?.metadata?.risk_level,
      ok: result.ok
    });
    if (!result.ok) {
      return {
...
export function isLeaveRequestMessage(message) {
...
function extractLeaveSlots(message) {
...
function extractEndTime(message, startTime, leaveDuration) {
...
function compactSlots(slots) {
  return Object.fromEntries(Object.entries(slots).filter(([, value]) => value !== null && value !== undefined && value !== ""));
}
...
function askForMissingSlots(missingSlots, slots) {
  const labels = {
...
function buildConfirmationMessage(slots) {
...
function resolveRelativeDateLabel(label) {
  const offset = {
    前天: -2,
    昨天: -1,
    今天: 0,
...
function parseChineseNumberDuration(text, unit) {
