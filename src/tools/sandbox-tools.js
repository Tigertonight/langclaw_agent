  ...
  return [
    {
      ...
      async execute(args, context) {
        return executeSafeCompute(args, context);
      }
    }
  ];
}

async function executeSafeCompute(args = {}, context = {}) {
...
function runWorker({ cwd, timeoutMs, payload }) {
...
function normalizeWorkerResult(childResult, durationMs) {
  if (!childResult.ok) {
    return {
      ok: false,
  ...
  try {
    const parsed = JSON.parse(String(childResult.stdout ?? "").trim());
    return {
      ...parsed,
...
function sandboxSummary(userId, sandboxDir, timeoutMs) {
  return {
    user_id: userId,
...
function summarizeForAudit(result) {
  return {
    ok: result.ok,
    error: result.error,
    duration_ms: result.duration_ms,
