...
async function findCustomerByName(customerName) {
  ...
  return customers.find((customer) => customer.name.includes(normalized) || normalized.includes(customer.name));
}

async function executeBusinessDataQuery(args, context) {
  ...
  const selectedRows = await enrichRows(args.resource, rows.slice(0, limit).map((row) => pickFields(row, fields)));
  return {
    ok: true,
    tool: "query_business_data",
    data: {
...
async function injectUserScope(rows, config, user, runtimeFilters = []) {
  if (config.scopeType === "self_user") {
    ...
    if (requestedApplicantIds.length > 0) {
      return rows.filter((row) => allowedIds.has(row.applicant_user_id));
    }
  ...
  return rows.filter((row) => allowed.has(row[config.scopeField]));
}

function applyFilters(rows, filters, allowedFields) {
...
async function resolveRuntimeFilters(filters, context) {
...
function matchFilter(row, filter, allowedFields) {
...
function compareDateLikeValues(left, right) {
...
function applySort(rows, sort, allowedFields) {
...
function computeMetrics(rows, metrics, allowedFields) {
...
async function getReportTreeUserIds(userId) {
  ...
  while (frontier.length > 0) {
    const next = employees
      .filter((employee) => Array.isArray(employee.direct_leader)
        && employee.direct_leader.some((leaderId) => frontier.includes(leaderId))
        && !ids.has(employee.userid))
...
async function getAllOrgUserIds() {
  ...
  return [...new Set([
    ...
    ...leaveRequests.map((request) => request.applicant_user_id)
  ].filter(Boolean))];
}

function getRequestedApplicantIds(filters = []) {
...
async function enrichDepartmentRows(rows) {
  const employees = await loadJson("data/wecom-users.json");
  const byUserId = new Map(employees.map((employee) => [employee.userid, employee]));
  return rows.map((row) => ({
    ...row,
...
function enrichReporting(reporting, byUserId) {
  if (!reporting || typeof reporting !== "object") return reporting;
  return {
    ...reporting,
    manager_profile: summarizeEmployee(byUserId.get(reporting.manager_userid), reporting.manager_userid),
    store_manager_profile: summarizeEmployee(byUserId.get(reporting.store_manager_userid), reporting.store_manager_userid)
  };
}

function summarizeEmployee(employee, fallbackUserId) {
  ...
  return [
    {
      ...
      async execute(args, context) {
        return executeBusinessDataQuery(args, context);
      }
    },
    {
      ...
      async execute(args, context) {
        const customers = await loadJson("data/customers.json");
        const allowed = new Set(context.user.accessible_customer_ids ?? []);
        const matched = customers
          .filter((customer) => allowed.has(customer.id))
    ...
    },
    {
      ...
      async execute(args) {
        ...
        const matched = orders
          .filter((order) => order.customer_name.includes(normalized) || normalized.includes(order.customer_name))
    ...
    },
    {
      ...
      async execute(args, context) {
        const reports = await loadJson("data/sales_reports.json");
        const department = args.department ?? context.user.department;
    ...
    },
    {
      ...
      async execute(args, context) {
        const request = normalizeLeaveRequestRecord({
          ...
          applicant_name: context.user.name,
        ...
        const existing = await loadJson(tableFile).catch(() => []);
        await saveJson(tableFile, existing.concat(request).map(normalizeLeaveRequestRecord));
...
function inferLeaveDuration(args) {
  if (args.leave_duration) return args.leave_duration;
...
function normalizeLeaveRequestRecord(record) {
  const startTime = normalizeLeaveDateTime(record.start_time, { fallbackTime: "09:00" });
  const endTime = normalizeLeaveEndTime(record.end_time, { startTime, leaveDuration: record.leave_duration });
  return {
    ...record,
...
function normalizeLeaveDuration(value, { startTime, endTime } = {}) {
...
function normalizeLeaveDateTime(value, { fallbackTime = "09:00" } = {}) {
  const text = String(value ?? "").trim();
  if (!text) return text;
...
function normalizeLeaveEndTime(value, { startTime, leaveDuration } = {}) {
  const text = String(value ?? "").trim();
...
function deriveEndTimeFromDuration(startTime, leaveDuration) {
...
function inferDurationFromRange(startTime, endTime) {
  const start = parseDateTime(startTime);
...
function normalizeMinutePrecision(value) {
