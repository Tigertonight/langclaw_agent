...
interface DealerSmokeResult {
  answer?: string;
  debug: {
    route?: { intent_code?: string; handler_type?: string; params?: Record<string, unknown> };
    tool_calls?: Array<{ resource?: string; name?: string }>;
    tool_results?: DealerToolResult[];
  };
}

interface DealerToolResult {
  resource?: string;
  row_count?: number;
  sample_rows?: DealerMetricRow[];
  rows?: DealerMetricRow[];
  data?: { rows?: DealerMetricRow[] };
}

interface DealerMetricRow {
  category?: string;
  severity?: string;
  [key: string]: unknown;
}
...
async function run(userId: string, message: string): Promise<DealerSmokeResult> {
  caseIndex += 1;
  return normalizeDealerSmokeResult(await agent.run({
    userId,
    wecomUserId: userId,
    message,
...
function expectRoute(result: DealerSmokeResult, intentCode: string, handlerType?: string): void {
  const route = result.debug.route ?? {};
...
function expectResource(result: DealerSmokeResult, resource: string): void {
...
function expectParam(result: DealerSmokeResult, key: string, expectedValue: string | RegExp): void {
...
function expectMetricCategory(result: DealerSmokeResult, category: string): void {
...
function expectMetricCategoryOrAnswer(result: DealerSmokeResult, category: string, answerPattern: RegExp): void {
  const rows = getResourceRows(result, "dealer_metrics");
...
function expectRows(result: DealerSmokeResult, resource: string): void {
...
function expectSeverity(result: DealerSmokeResult, severity: string): void {
  const rows = getResourceRows(result, "dealer_metrics");
  if (!rows.some((row) => row.severity === severity)) throw new Error(`expected dealer_metrics severity ${severity}`);
}

function getResourceRows(result: DealerSmokeResult, resource: string): DealerMetricRow[] {
...
function normalizeDealerSmokeResult(value: Record<string, unknown>): DealerSmokeResult {
  return {
    answer: typeof value.answer === "string" ? value.answer : undefined,
    debug: value.debug && typeof value.debug === "object" ? value.debug as DealerSmokeResult["debug"] : {}
  };
}
