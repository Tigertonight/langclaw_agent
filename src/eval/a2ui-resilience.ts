import { buildA2UIResponse, ENVELOPE_LIMITS } from "../a2ui/adapter.js";
import { IdempotencyCache } from "../a2ui/idempotency-cache.js";
import { sharedMetrics } from "../security/observability.js";
import type { SurfacePlugin } from "../a2ui/plugins/index.js";
...
// ─── 1. envelope 大小限制 ─────────────────────────────────────────────
{
  ...
  const messages = buildA2UIResponse({ result: { run_id: "r1" }, plugins: [explodingPlugin] });
...
// ─── 2. envelope 嵌套深度限制 ─────────────────────────────────────────
{
  ...
  const deepPlugin: SurfacePlugin<unknown> = {
    ...
    build: () => ({
      ...
      root: "root",
      data: {},
      components: [{ id: "deep", component: { Card: { children: [], data: nested } } }] as A2UIComponentInstance[]
    })
  };
  const before = readMetric("envelope_rejected_total");
  const messages = buildA2UIResponse({ result: { run_id: "r2" }, plugins: [deepPlugin] });
...
// ─── 3. envelope 序列化字节超限 ───────────────────────────────────────
{
  ...
  const fatPlugin: SurfacePlugin<unknown> = {
    ...
    build: () => ({
      ...
      root: "root",
      data: { huge },
      components: [{ id: "n", component: { Text: { text: { literalString: "ok" } } } }] as A2UIComponentInstance[]
    })
  };
  const before = readMetric("envelope_rejected_total");
  const messages = buildA2UIResponse({ result: { run_id: "r3" }, plugins: [fatPlugin] });
...
// ─── 4. 客户端能力协商：不支持的组件被降级为 Text ──────────────────────
{
  const exoticPlugin: SurfacePlugin<unknown> = {
    ...
    build: () => ({
      ...
      data: {},
      components: [
        { id: "supported", component: { Text: { text: { literalString: "ok" } } } } as unknown as A2UIComponentInstance,
        { componentName: "FancyChart3D", id: "exotic", component: { FancyChart3D: { points: [1, 2, 3] } } } as unknown as A2UIComponentInstance
      ]
    })
  };
  ...
  else if (downgraded.componentName !== "Text") failures.push(`downgrade: expected componentName=Text, got ${JSON.stringify(downgraded.componentName)}`);
}

// ─── 5. IdempotencyCache：基本命中 + TTL ────────────────────────────────
{
  const cache = new IdempotencyCache({ ttlMs: 200, maxEntries: 10 });
...
if (failures.length > 0) {
  console.error(`FAIL a2ui resilience: ${failures.length} failures`);
  for (const f of failures) console.error(` - ${f}`);
  process.exitCode = 1;
} else {
  console.log("PASS a2ui resilience (6 cases)");
}
