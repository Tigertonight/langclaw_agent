...
export function inferDealerEvidenceFacts(message, route) {
...
export function planDealerEvidenceFollowUp({ message, route, agentState, previousCalls = [] }) {
  const missingFacts = agentState?.missing_facts ?? [];
  const needed = missingFacts.filter((fact) => fact.startsWith("dealer_") && EVIDENCE_TO_RESOURCE[fact]);
...
function buildEvidenceArgs({ spec, message, fact }) {
  const filters = [];
...
function inferStore(message) {
  const text = String(message ?? "");
  return STORE_ALIASES.find(([, name, aliases]) => text.includes(name) || aliases.some((alias) => text.includes(alias)))
    ? (() => {
      const [, name] = STORE_ALIASES.find(([, fullName, aliases]) => text.includes(fullName) || aliases.some((alias) => text.includes(alias)));
      return { name };
    })()
    : null;
}

function isDealerRoute(route) {
