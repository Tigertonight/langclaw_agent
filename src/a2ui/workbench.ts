/**
 * A2UI Workbench — Phase 4 新增
 *
 * 企业经营工作台的 A2UI Surface 构建器。
 * 将复杂业务数据渲染为结构化 A2UI envelope，而不只靠文本承载。
 *
 * 工作台组件：
 ...
 *
 * 设计原则：
 * 1. 每个 Surface builder 接收业务数据，返回标准 A2UIEnvelope
 * 2. surfaceId 唯一标识一个渲染面板，支持多面板并存
 * 3. 所有 builder 函数纯函数，不依赖外部状态
 * 4. 与 A2UIEnvelope 协议兼容（createSurface + updateComponents + updateDataModel）
 */

import type { JsonObject, JsonValue } from "../types/agent-contracts.js";
import type { A2UIEnvelope, A2UIComponentInstance } from "./types.js";
 ...
 */
export function buildToolCatalogSurface(surfaceId: string, data: ToolCatalogSurfaceData): A2UIEnvelope {
...
export function buildMetricCardsSurface(surfaceId: string, metrics: MetricCard[], title?: string): A2UIEnvelope {
  const components: A2UIComponentInstance[] = [];
...
export function buildEvidenceSurface(surfaceId: string, evidence: EvidenceData): A2UIEnvelope {
  const components: A2UIComponentInstance[] = [];
...
export function buildPendingActionSurface(surfaceId: string, actions: PendingActionData[]): A2UIEnvelope {
  const components: A2UIComponentInstance[] = [];
...
export function buildTaskTrackingSurface(surfaceId: string, tasks: TaskTrackingEntry[], title?: string): A2UIEnvelope {
  const components: A2UIComponentInstance[] = [];
