...
export function buildSystemPrompt(registry) {
  ...
  "reasoning": "<一句话理由>"
    ...
    "9. **滑窗仅作上下文**：session_state.recent_messages（最近用户原话）和 recent_routes（最近的查询历史）只是参考，用来理解代词指代和用户的关注脉络。不要把 recent_routes 里的 intent_code/params 当成『必须命中』的目标——除非当前消息显式指向它们。",
    "",
    "few-shot 例子：",
    exampleLines
  ].join("\n");
